...
public class ConsumerThread implements Runnable {
	...
	private final HttpClient m_kairosClient = KairosDBClient.getInstance();

	private final String m_topic;
	private final KafkaStream<byte[], byte[]> m_stream;
	private final int m_threadNumber;
	private TopicParserMetric m_topicParser;
	private static final Charset UTF8 = Charset.forName("UTF-8");

	public ConsumerThread(String topic, KafkaStream<byte[], byte[]> stream, int threadNumber) {
		m_topic = topic;
		m_stream = stream;
		m_threadNumber = threadNumber;
	}

	public void setTopicParser(TopicParserMetric parser) {
		...
		for (MessageAndMetadata<byte[], byte[]> messageAndMetadata : m_stream) {
			try {
				...
				if (jsonObject.has("measure")) {
					...
					metric.addMetric(metricName).addTag("client", metricName)
					 ...
					 * 1459256714,"device":"Downstairs","manufacturer":
					 * "Honeywell"} Message :
					...
					metric.addMetric(metricName).addTag("client", metricName)
							...
							.addTag("manufacturer", jsonObject.getString("manufacturer")).addDataPoint(timestamp);

				}
				storeMetrics(metric);

				/*
				 * DataPointSet set = m_topicParser.parseTopic(m_topic,
				 * messageAndMetadata.key(), messageAndMetadata.message()); for
				 * (DataPoint dataPoint : set.getDataPoints()){
				 * //m_kairosClient.putDataPoint(set.getName(), set.getTags(),
				 * dataPoint); }
				 */
			}
			/*
			 * catch (DatastoreException e) { // TODO: rewind to previous
			 * message to provide consistent consumption
